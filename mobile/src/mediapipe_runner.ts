...
export class MediapipeRunnerMobile extends MediapipeRunner<Frame> {
  ...
  async loadHandTrackModel() {
    ...
    try {
      this.hand_detector = await loadTensorflowModel({
        url: this.hand_detector_path,
      });
      console.log("Hand Detector model loaded!");
      this.hand_landmarker = await loadTensorflowModel({
        url: this.hand_landmarker_path,
      });
  ...
  async runHandTrackModel(video: Frame): Promise<DataGestures> {
    ...
    try {
      ...
      const [palmLocations, palmScores] = this.hand_detector.runSync([
        resizedUri,
      ]);
      const THRESHOLD = 0.5;
      const palms = [];
