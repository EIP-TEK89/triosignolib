...
# Script pour publier le package triosigno-lib-web
...
echo -e "${YELLOW}Publication du package web...${NC}"
...
# Installer les dépendances pour web
cd "$WEB_DIR" || {
  echo -e "${RED}Erreur: Impossible d'accéder au répertoire $WEB_DIR${NC}"
  exit 1
}
...
# Publier le package web si on n'est pas en mode dry-run
if [ "$DRY_RUN" = true ]; then
  echo -e "${YELLOW}Mode simulation - Pas de publication pour triosigno-lib-web${NC}"
else
  echo -e "${YELLOW}Publication du package triosigno-lib-web...${NC}"
  npm publish --access public
  if [ $? -ne 0 ]; then
    echo -e "${RED}✗ Échec de la publication du package triosigno-lib-web${NC}"
    exit 1
  fi
  echo -e "${GREEN}✓ Package triosigno-lib-web publié avec succès${NC}"
fi

echo -e "${GREEN}✓ Traitement du package web terminé avec succès${NC}"
